...
const { data, status, error, refresh } = await useFetch<{ items: NotificationItem[], unreadCount: number }>('/api/student/notifications')
...
const openNotification = async (item: NotificationItem) => {
  try {
    if (!item.isRead) await $fetch(`/api/student/notifications/${item.id}/read`, { method: 'PATCH' })
...
<template>
  <UDashboardPanel id="student-notifications-page">
    ...
    <template #body>
      ...
      <div v-else class="mx-auto max-w-3xl divide-y divide-default rounded-xl border border-default bg-default">
        <button
          v-for="item in data?.items || []"
          :key="item.id"
          type="button"
          class="block w-full px-5 py-4 text-left transition-colors hover:bg-elevated"
        ...
        <UEmpty v-if="!data?.items?.length && status !== 'pending'" icon="i-lucide-bell" title="ยังไม่มีการแจ้งเตือน" />
      </div>
    </template>
  </UDashboardPanel>
</template>
