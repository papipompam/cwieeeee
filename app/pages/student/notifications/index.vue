...
<template>
  <UDashboardPanel id="student-notifications-page">
    ...
    <template #body>
      <UAlert v-if="error" color="error" variant="subtle" icon="i-lucide-circle-alert" title="ไม่สามารถโหลดการแจ้งเตือนได้" :description="error.message" />
      <div v-else class="mx-auto max-w-3xl divide-y divide-divider rounded-panel border border-divider bg-canvas shadow-panel overflow-hidden">
        <button
          ...
          :key="item.id"
          type="button"
          class="block w-full px-5 py-4 text-left transition-colors hover:bg-surface cursor-pointer"
