export default defineEventHandler(async (event) => {
  ...
  if (!reason) throw createError({ statusCode: 400, message: 'กรุณาระบุเหตุผลที่ส่งกลับแก้ไข' })
  ...
  if (!latestDoc || !members.every(member => member.status === 'DOCUMENT_UNDER_REVIEW')) throw createError({ statusCode: 400, message: 'สามารถส่งกลับแก้ไขได้เฉพาะเอกสารที่อยู่ระหว่างตรวจสอบเท่านั้น' })
  const result = await prisma.$transaction(async (tx) => {
    await tx.$executeRaw`SELECT pg_advisory_xact_lock(${group?.id ?? request.id}, 2)`
    const document = await tx.requestDocument.updateMany({ where: { id: latestDoc.id, status: { in: ['UPLOADED', 'UNDER_REVIEW'] } }, data: { status: 'RETURNED_FOR_REVISION', reviewerNote: reason, reviewedAt: new Date() } })
    const requestIds = members.map(member => member.id)
    const updated = await tx.cooperativeRequest.updateMany({ where: { id: { in: requestIds }, status: 'DOCUMENT_UNDER_REVIEW' }, data: { status: 'RETURNED_FOR_REVISION', returnedReason: reason } })
    ...
    await tx.notification.createMany({ data: members.map(member => ({ userId: member.companyApplication.studentUserId, title: 'เอกสารถูกส่งกลับเพื่อแก้ไข', message: `เอกสารสำหรับ ${request.companyName} ต้องแก้ไข: ${reason}`, link: '/student/applications' })) })
